...
function initLevel(lvl) {
  ...
  gameOver = false; levelComplete = false; timer = 0; isAnswering = false;
  ...
  // Get random question filtered by difficulty and topic
  let filteredQuestions = questionPool.filter(q => q.diff === gameDifficulty);
  if (selectedTopic !== 'all') {
    filteredQuestions = filteredQuestions.filter(q => q.topic === selectedTopic);
  }
  if (filteredQuestions.length === 0) filteredQuestions = questionPool.filter(q => q.diff === gameDifficulty);

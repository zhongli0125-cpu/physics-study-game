...
// Reduced speed from 4 to 3 for better control
let fireboy = { x: 50, y: 300, w: 24, h: 32, vx: 0, vy: 0, speed: 3, jumpPower: 11, onGround: false, alive: true, color: "#ff4500", inDoor: false };
let watergirl = { x: 100, y: 300, w: 24, h: 32, vx: 0, vy: 0, speed: 3, jumpPower: 11, onGround: false, alive: true, color: "#1e90ff", inDoor: false };
let keys = {};
...
function initLevel(lvl) {
  // Use improved level generator if available
  if (typeof initLevelImproved !== 'undefined') {
    initLevelImproved(lvl);
    return;
  }
  
    const randomQ = questionPool[Math.floor(Math.random() * questionPool.length)];
    questionGate = { x: 500, y: 200, w: 60, h: 80, question: randomQ.q, options: randomQ.opts, answer: randomQ.ans, passed: false };
  ...
  platforms.push({ x: 500, y: 280, w: 100, h: 20, type: 'platform' });
    const randomQ = questionPool[Math.floor(Math.random() * questionPool.length)];
    questionGate = { x: 400, y: 200, w: 60, h: 80, question: randomQ.q, options: randomQ.opts, answer: randomQ.ans, passed: false };
...
function updatePlayer(player, leftKey, rightKey, jumpKey, otherPlayer) {
  ...
  platforms.forEach(plat => {
    if (player.x + player.w > plat.x && player.x < plat.x + plat.w && 
        player.y + player.h > plat.y && player.y + player.h < plat.y + 20 && player.vy >= 0) {
  ...
  // Check collision with other player (can stand on each other!)
  if (otherPlayer.alive) {
    if (player.x + player.w > otherPlayer.x && player.x < otherPlayer.x + otherPlayer.w &&
        player.y + player.h > otherPlayer.y && player.y + player.h < otherPlayer.y + 10 && player.vy >= 0) {
      player.y = otherPlayer.y - player.h;
      player.vy = 0;
      player.onGround = true;
    }
  }
  ...
  if (keys[jumpKey] && player.onGround) { 
    ...
    player.onGround = false; 
    if (typeof playJump !== 'undefined') playJump(); 
  }
  ...
  if (player.y > canvas.height) { 
    player.alive = false; 
    if (typeof playDeath !== 'undefined') playDeath(); 
  }
  ...
  if (player === fireboy) {
    waterZones.forEach(zone => { 
      if (checkCollision(player, zone)) { 
        player.alive = false; 
        if (typeof playDeath !== 'undefined') playDeath(); 
        if (typeof createParticles !== 'undefined') createParticles(player.x + player.w/2, player.y + player.h/2, '#1e90ff', 20); 
      } 
    });
  } else {
    lavaZones.forEach(zone => { 
      if (checkCollision(player, zone)) { 
        player.alive = false; 
        if (typeof playDeath !== 'undefined') playDeath(); 
        if (typeof createParticles !== 'undefined') createParticles(player.x + player.w/2, player.y + player.h/2, '#ff4500', 20); 
      } 
    });
  }
  ...
  greenZones.forEach(zone => { 
    if (checkCollision(player, zone)) { 
      player.alive = false; 
      if (typeof playDeath !== 'undefined') playDeath(); 
      if (typeof createParticles !== 'undefined') createParticles(player.x + player.w/2, player.y + player.h/2, '#32cd32', 20); 
    } 
  });
...
function update() {
  ...
  // Pass other player so they can collide with each other
  updatePlayer(fireboy, 'a', 'd', 'w', watergirl);
  updatePlayer(watergirl, 'ArrowLeft', 'ArrowRight', 'ArrowUp', fireboy);
  ...
  fireboy.inDoor = false; 
  watergirl.inDoor = false;
...
function draw() {
  ...
  // Draw platforms
  platforms.forEach(plat => { 
    ctx.fillStyle = plat.type === 'ground' ? '#2d2d44' : '#4a4a6a'; 
  ...
  lavaZones.forEach(zone => { 
    ...
    let offset = Math.floor(timer / 10) % 20; 
    for (let i = 0; i < zone.w; i += 20) ctx.fillRect(zone.x + i + offset, zone.y, 10, zone.h); 
  });
  ...
  waterZones.forEach(zone => { 
    ...
    let offset = Math.floor(timer / 10) % 20; 
    for (let i = 0; i < zone.w; i += 20) ctx.fillRect(zone.x + i + 10 - offset, zone.y, 10, zone.h); 
  });
  ...
  if (questionGate && !questionGate.passed) { 
    let glow = Math.sin(timer / 20) * 0.3 + 0.7; 
    ctx.globalAlpha = glow; 
    ctx.fillStyle = '#ffd700'; 
    ctx.fillRect(questionGate.x, questionGate.y, questionGate.w, questionGate.h); 
    ctx.globalAlpha = 1; 
    ctx.fillStyle = '#000'; 
    ctx.font = '20px Arial'; 
  ...
  collectibles.forEach(item => { 
    if (!item.collected) { 
      let pulse = Math.sin(timer / 15) * 2 + 8; 
      ctx.fillStyle = item.type === 'fire' ? '#ff4500' : '#1e90ff'; 
      ctx.beginPath(); 
      ctx.arc(item.x + 10, item.y + 10, pulse, 0, Math.PI * 2); 
      ctx.fill(); 
    } 
  });
  ...
  if (doors.fire) { 
    ...
    ctx.fillStyle = '#000'; 
    ctx.fillRect(doors.fire.x + 5, doors.fire.y + 10, 20, 30); 
  }
  ...
  if (doors.water) { 
    ...
    ctx.fillStyle = '#000'; 
    ctx.fillRect(doors.water.x + 5, doors.water.y + 10, 20, 30); 
  }
  ...
  if (fireboy.alive) { 
    ...
    ctx.fillStyle = '#fff'; 
    ctx.fillRect(fireboy.x + 6, fireboy.y + 8, 5, 5); 
    ctx.fillRect(fireboy.x + 13, fireboy.y + 8, 5, 5); 
  }
  ...
  if (watergirl.alive) { 
    ...
    ctx.fillStyle = '#fff'; 
    ctx.fillRect(watergirl.x + 6, watergirl.y + 8, 5, 5); 
    ctx.fillRect(watergirl.x + 13, watergirl.y + 8, 5, 5); 
  }
  
  // Update status
  const statusEl = document.getElementById('gameStatus');
  if (levelComplete) { 
    statusEl.textContent = '🎉 Level Complete! Moving to next level...'; 
...
function askQuestion() {
  ...
  if (parseInt(answer) - 1 === questionGate.answer) {
    ...
    // Don't show the correct answer - just say try again!

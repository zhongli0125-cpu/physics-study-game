...
app.post('/ask-question', async (req, res) => {
  try {
    ...
    const simpleAnswers = {
      'Motion': 'Motion involves concepts like velocity, acceleration, and displacement. Check the lesson above for detailed explanations and formulas!',
      'Newton Laws': 'Newton\'s Laws explain force, mass, and acceleration. The three laws are: inertia (F=0), F=ma, and action-reaction pairs.',
      'Energy': 'Energy comes in many forms: kinetic (½mv²), potential (mgh), thermal, and more. Energy is always conserved!',
      'Electricity': 'Electricity involves voltage, current, and resistance. Remember Ohm\'s Law: V=IR. Power is P=VI.',
      'Waves': 'Waves have frequency, wavelength, and amplitude. Wave speed v=fλ. Waves can be transverse or longitudinal.',
      'Modern Physics': 'Modern physics includes relativity (E=mc²), quantum mechanics, and particle physics. Time and space are relative!'
    };
    
    const answer = simpleAnswers[topic] || `Great question about ${topic}! Review the lesson content above for detailed information about this topic.`;
    
    console.log('Sending simple answer');
    res.json({ answer });
  } catch (error) {
